...
export class InMemoryCheckInRepository implements CheckInRepository {
    ...
    async countByUserId(userId: string): Promise<number> {
        return this.items.filter((item) => item.users_id === userId).length
    }
    async findManyByUserId(userId: string, page:number): Promise<CheckIn[]> {
        return this.items.filter(item => item.users_id === userId).slice((page - 1)*20 , (page * 20))
    }
    ...
    async findByUserIdOnDate(userId: string, date: Date): Promise<CheckIn | null> {
        ...
        const checkInOnSameDay = this.items.find((checkIn) => {
            ...
            return checkIn.users_id === userId && isOnSameDay
        })
